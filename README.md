...
- The `/detections` folder comprises the detection data from each detector (e.g. `/detections/focus/detections.csv`), the circuli spacings (`detections/circuli/circuli_spacings.csv`), and subdirectories containing the images with detection boxes drawn if `--plot_dets` is set to `True` (e.g. `/detections/focus/detection_images`)
