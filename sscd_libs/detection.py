...
def detections_as_df(detections_tf, img_orig_wh, img_id, class_names):
    ...
    id_classes_scores = pd.DataFrame(
        {"img_id": img_id, "class_name": class_names, "score": scores}
    )
    ...
    det_areas = (detections_df.xmax - detections_df.xmin) * (
...
def plot_detections(
    ...
    if fig_w is None or fig_h is None:
        fig_w = min(img.shape[1] * 0.15, 100)
        fig_h = min(img.shape[0] * 0.15, 100)
    ...
    if draw_ann:
        ...
        if anns_sepPlot:
            ...
            for i in range(len(anns)):
                ...
                if draw_det_num:
                    ax2.text(
                        ann_bbxs_centres[i][0],
                        ann_bbxs_centres[i][1] - 2,
                        i + 1,
                        fontsize="small",
                        c="limegreen",
                        ha="center",
